...
def minTempWeatherData(sc:SparkContext, filePath):
    ...
    minTemps = parsedLines.filter(lambda x: 'TMAX' in x[1]) # filtering the entryType with TMIN
    stationTemps = minTemps.map(lambda x: (x[0], x[2])) # form a key:value RDD with (key: stationId,value: temperature)
    minTemps = stationTemps.reduceByKey(lambda x,y: max(x,y)) # reduce to the lowest temparature amoung all the stationId for TMIN data
